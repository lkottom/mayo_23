...
def process_wsi_dino(wsi_path, 
    ...
    try:
        ...
        for (x, y) in selected:
            ...
            patch_size_40x = int((objective_power/40.)*output_patch_size)
            patch = wsi_obj.read_region((x, y), 0, (patch_size_40x, patch_size_40x))
...
if __name__ == "__main__":
    n_word_list = [500, 1000]
    patch_size = 32
    
    for n_word in n_word_list:
        keywords = ["train", "test"]
        num_of_words = n_word
        
        for keyword in keywords:
            wsi_dir = f'/mayo_atlas/home/m296984/main_pipeline/{keyword}/{keyword}_atlas_tissue'
            save_dir = f'/mayo_atlas/home/m296984/visual_dictionary_pipeline/{keyword}/{num_of_words}_words_per_slide_{patch_size}'
            os.makedirs(save_dir, exist_ok=True)
            
            words_per_patch = num_of_words/20
            
            # print(f'Processing {keyword} set for {n_word} number of words per slide')
